...
<tr>
        ...
        <td><input type="text" name="sorting" size="5" value="1" class="edit bk"><a href="#" class="hintanchor" onMouseover="showhint('Индекс сортировки служит для дополнительной и более точной сортировки новостей', this, event, '300px')">[?]</a></td>
    </tr>
    <tr>
...
elseif( $action == "doaddnews" ) {
	...
	$sorting = isset( $_POST['sorting'] ) ? intval( $_POST['sorting'] ) : 1;
	...
	$db->query( "INSERT INTO " . PREFIX . "_post (date, autor, short_story, full_story, xfields, title, descr, keywords, category, alt_name, allow_comm, approve, allow_main, fixed, sorting, allow_br, symbol, tags, metatitle) values ('$thistime', '{$member_id['name']}', '$short_story', '$full_story', '$filecontents', '$title', '{$metatags['description']}', '{$metatags['keywords']}', '$category_list', '$alt_name', '$allow_comm', '$approve', '$allow_main', '$news_fixed', '$sorting', '$allow_br', '$catalog_url', '{$_POST['tags']}', '{$metatags['title']}')" );
