...
function news_image111($row_short_story){

			$not_tb_images = array();
			preg_match_all('/(img|src)=("|\')[^"\'>]+/i', $row_short_story, $media);
			...
			foreach($data as $url) {
				...
				if (isset($info['extension'])) {
					...
					if (($info['extension'] == 'jpg') || ($info['extension'] == 'jpeg') || ($info['extension'] == 'gif') || ($info['extension'] == 'png')) array_push($not_tb_images, $url);
				}
			}

return $not_tb_images[0];
}
...
if( $action == "list" ) {
	...
	$db->query( "SELECT p.id, p.date, p.title, p.category, p.autor, p.alt_name, p.comm_num, p.approve, p.fixed, p.sorting, e.news_read, e.votes FROM " . PREFIX . "_post p LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) " . $where . " ORDER BY " . $order_by . " LIMIT $start_from,$news_per_page" );
...
<form action="?mod=editnews&amp;action=list" method="GET" name="optionsbar2" id="optionsbar">
<input type="hidden" name="mod" value="editnews">
<input type="hidden" name="action" value="list">

<div style="padding-top:5px;padding-bottom:2px" name="advancedsearch2" id="advancedsearch2">
<table width="100%">

		<td style="padding-left:5px;"><input class="edit bk" name="search_field" value="{$search_field}" type="text" size="35"> Найти</td>
...
$row_short_story1 = $row['short_story'];
...
$news_image111 = news_image111($row_short_story1);
$news_image222 = str_replace( "/thumbs/", "/", $news_image111);
	
	echo <<<HTML
<link rel="stylesheet" type="text/css" media="all" href="engine/skins/calendar-blue.css" title="win2k-cold-1" />
<link rel="stylesheet" type="text/css" href="engine/skins/chosen/chosen.css"/>

<a href="{$news_image222}" target="_blank"><img src="{$news_image111}" width="150"></a>
<div style="padding-top:5px;padding-bottom:2px;">
<table width="100%">
    <tr>
        <td width="4"><img src="engine/skins/images/tl_lo.gif" width="4" height="4" border="0"></td>
