		...
		if( isset($view_template) AND $view_template == "print" ) {
			...
			if( $anzahl_seiten > 1 ) {
				...
				if( $news_page > 1 ) {
					...
					if( $config['allow_alt_url'] == "yes" ) {
						if ( $pages == 1 ) $prevpage = "<a href=\"" . $full_link . "\">" . $lang['news_prev'] . "</a> | ";
						else $prevpage = "<a href=\"" . $short_link . "page," . $pages . "," . $row['alt_name'] . ".html\">" . $lang['news_prev'] . "</a> | ";
					} else {
						if ( $pages == 1 ) $prevpage = "<a href=\"" . $full_link. "\">" . $lang['news_prev'] . "</a> | ";
						else $prevpage = "<a href=\"$PHP_SELF?newsid=" . $row['id'] . "&amp;news_page=" . $pages . "\">" . $lang['news_prev'] . "</a> | ";
					}
				}
...
//					$db->query( "SELECT id, date, short_story, xfields, title, category, alt_name FROM " . PREFIX . "_post WHERE `category` = '{$row['category']}' ORDER BY RAND() LIMIT 6");
					$first_show = false;
					...
					$db->query( "SELECT id, date, short_story, xfields, title, category, alt_name FROM " . PREFIX . "_post WHERE {$where_category}{$allowed_cats} id != " . $row['id'] . " AND approve=1" . $where_date . " LIMIT " . $config['related_number'] );
				}
