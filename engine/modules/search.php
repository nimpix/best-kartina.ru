  ...
  <tr>
    <td class="search">
      <div style="margin:10px;">
                <input type="text" name="story" id="searchinput" value="$findstory" class="textin" style="width:250px" />
                <input type="submit" class="search_button" name="dosearch" id="dosearch" value="Ïîèñê" onclick="javascript:list_submit(-1); return false;">
	...
	if( $subaction == "search" ) {
		...
		if( (empty( $story ) or ($story_maxlen < $config['search_length_min'])) and (empty( $searchuser ) or (strlen( $searchuser ) < $config['search_length_min'])) ) {
			...
			if ($config['full_search']) {
					...
					// 						  '1' => "MATCH(text) AGAINST ('{story}')", // ˜˜˜˜˜˜ ˜˜˜˜˜˜ ˜ ˜˜˜˜˜˜˜˜˜˜˜˜
					// 						  '2' => "title LIKE '%{story}%'", // ˜˜˜˜˜˜ ˜˜˜˜˜˜ ˜ ˜˜˜˜˜˜˜˜˜˜˜ ˜˜˜˜˜˜˜˜˜
					// 						  '3' => "title LIKE '%{story}%'" ); // ˜˜˜˜˜˜ ˜˜˜˜˜˜ ˜ ˜˜˜˜˜˜˜˜˜˜ ˜˜˜˜˜˜
					$titleonly_where = array ('0' => "title LIKE '%{story}%' OR keywords LIKE '%{story}%'", // ˜˜˜˜˜˜ ˜˜˜˜˜˜ ˜ ˜˜˜˜˜˜˜
											  '1' => "title LIKE '%{story}%' OR keywords LIKE '%{story}%'", // ˜˜˜˜˜˜ ˜˜˜˜˜˜ ˜ ˜˜˜˜˜˜˜˜˜˜˜˜
											  '2' => "title LIKE '%{story}%' OR keywords LIKE '%{story}%'", // ˜˜˜˜˜˜ ˜˜˜˜˜˜ ˜ ˜˜˜˜˜˜˜˜˜˜˜ ˜˜˜˜˜˜˜˜˜
											  '3' => "title LIKE '%{story}%' OR keywords LIKE '%{story}%'" ); // ˜˜˜˜˜˜ ˜˜˜˜˜˜ ˜ ˜˜˜˜˜˜˜˜˜˜ ˜˜˜˜˜˜
			...
			if( ! $found_result ) {
				...
				while ( $row = $db->get_row( $sql_result ) ) {
					...
					if( $titleonly == 2 ) {
						...
						$full_link1 = $config['http_home_url'] . $row['id'] . "-" . $row['alt_name'] . ".html";
						$tpl->set( '{full-link}', $full_link1 );



						$go_page = $config['http_home_url'] . "user/" . urlencode( $row['autor'] ) . "/";
						$go_page = "onclick=\"ShowProfile('" . urlencode( $row['autor'] ) . "', '" . htmlspecialchars( $go_page, ENT_QUOTES, $config['charset'] ) . "', '" . $user_group[$member_id['user_group']]['admin_editusers'] . "'); return false;\"";
						...
						if( $row['output_comms'] == '1' ) {
							...
							if( ! $row['is_register'] ) {
								...
								if( $row['gast_email'] != "" ) {
									$tpl->set( '{result-author}', "<a href=\"mailto:".htmlspecialchars($row['gast_email'], ENT_QUOTES, $config['charset'])."\">" . stripslashes( $row['autor'] ) . "</a>" );
								} else {
							...
							if ( preg_match( "#\\{result-text limit=['\"](.+?)['\"]\\}#i", $tpl->copy_template, $matches ) ) {
								$count= intval($matches[1]);
					
								$row['story'] = str_replace( "</p><p>", " ", $row['story'] );
								$row['story'] = strip_tags( $row['story'], "<br>" );
