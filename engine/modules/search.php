  ...
  <tr>
    <td class="search">
      <div style="margin:10px;">
                ...
                <input type="submit" class="search_button" name="dosearch" id="dosearch" value="Поиск" onclick="javascript:list_submit(-1); return false;">
    ...
    if( $subaction == "search" ) {
        ...
        if( (empty( $story ) or ($story_maxlen < $config['search_length_min'])) and (empty( $searchuser ) or (strlen( $searchuser ) < $config['search_length_min'])) ) {
            ...
            if ($config['full_search']) {
                ...
                $titleonly_where = array ('0' => "title LIKE '%{story}%' OR keywords LIKE '%{story}%' OR xfields LIKE '%{story}%' OR short_story LIKE '%{story}%'
					OR full_story LIKE '%{story}%'");
