...
if (!isset ($tagFace['description'])) $tagFace['description'] = "";
if($config['version_id'] < 9.6)
{
echo <<<HTML
<tr>
	<td valign="top" style="padding:2px;">Описание категории:</td>
...
$(function(){
	$('#description').tinymce({
		...
		setup : function(ed) {
			...
			onclick : function() {
				...
				window.open('?mod=files&action=quick&area=description&wysiwyg=yes&author={$member_id['name']}&news_id={$row['id']}', '_Addimage', 'toolbar=0,location=0,status=0, left=0, top=0, menubar=0,scrollbars=yes,resizable=0,width=640,height=550');
			}
	           });
    ...
    <textarea name="description" id="description" style="width:98%;height:300px;">{$tagFace['description']}</textarea></td></tr>
HTML;
}
...
function tag_typograf() {
    ...
    <textarea name="description" id="description" class="wysiwygeditor" style="width:98%;height:300px;">{$tagFace['description']}</textarea></td></tr>
HTML;
}
