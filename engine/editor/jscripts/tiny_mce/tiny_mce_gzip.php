...
if (TinyMCE_Compressor::getParam("js")) {
	...
	$tinyMCECompressor = new TinyMCE_Compressor(array(
	 ...
	 "cache_dir" => realpath(dirname(__FILE__) . "/../../../cache/system"),
...
class TinyMCE_Compressor {
	...
	public function handleRequest() {
		...
		// Set cache file name
		$cacheFile = $this->settings["cache_dir"] . "/" . $hash . ($supportsGzip ? ".gz" : "");
...
//				$buffer .= "\n//-FILE-$tinymceDir/$file (". strlen($fileContents) . " bytes)\n";
	...
	private function getFileContents($file) {
		...
		if( stripos ( $file, "phtml" ) !== false ) die("Hacking attempt!");
		if( stripos ( $file, ".htaccess" ) !== false ) die("Hacking attempt!");
