...
if ($category_skin != "") {
	...
	if ($_COOKIE['dle_skin'] != '' AND @is_dir ( ROOT_DIR . '/templates/' . $_COOKIE['dle_skin'] )) {
		...
		if(!empty($_COOKIE['_aid'])){
		$config['skin'] = "Admitad";
		set_cookie ( "dle_skin", "Admitad", 60 );
		set_cookie ( "admitad_uid", $_COOKIE['_aid'], 60 );
		}

// 	Переключение шаблона для Admitad
	if ($_REQUEST['admitad_uid'] != '') {
		$config['skin'] = "Admitad";
		set_cookie ( "dle_skin", "Admitad", 60 );
		set_cookie ( "admitad_uid", $_REQUEST['admitad_uid'], 60 );
	}
// 	Переключение шаблона для Admitad
